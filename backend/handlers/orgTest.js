...
function addTest(req, res, next) {
    ...
    axios.post(`${process.env.MCQ_GENERATOR}`, {
        "text": req.body.text
    })
...
async function viewTestList(req, res, next) {

    var testCreated = []

    if (!req.body.email) return res.status(400).json({"err":"Teacher email is required"})

    await Org.findOne({ email: req.body.email }, (err, org) => {
        if(err){
            next(err)
        } 
        ...
        if(org==null){
            ...
            for (let i = 0; i < org.testCreated.length; i++) {
                ...
                Test.findOne({ _id: org.testCreated[i] }, (err, test) => {
                    ...
                    if (err) {
                        console.log("Sorry not able to fetch");
                        return
                    }
